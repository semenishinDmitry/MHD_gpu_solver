...
// Scratch for MUSCL-GLM RHS. Owned by TimeIntegratorWorkspace.
struct RHSWorkspace {
    PrimitiveField prim;
...
// Second-order MUSCL + HLL + Dedner GLM spatial operator L(U).
// Requires grid.ng >= 2 and filled ghost cells.
//
// Hyperbolic GLM: fluxes carry psi <-> B_n coupling at speed c_h.
// Parabolic damping source: d(psi)/dt += -(c_h^2 / c_p^2) psi,
// with c_p^2 = c_h * min(dx,dy) / glm_alpha  (Dedner).
                        ...
                        SlopeLimiter limiter)
{
    ...
    // X-interfaces at i-1/2 for i in [i_begin, i_end] (covers left+right faces of interior).
    ...
    // Y-interfaces at j-1/2 for j in [j_begin, j_end].
    for (int j = grid.j_begin(); j <= grid.j_end(); ++j) {
        for (int i = grid.i_begin(); i < grid.i_end(); ++i) {
    ...
    // Dedner: c_p^2 = c_h * h / alpha  =>  c_h^2 / c_p^2 = c_h * alpha / h
    ...
    for (int j = grid.j_begin(); j < grid.j_end(); ++j) {
        for (int i = grid.i_begin(); i < grid.i_end(); ++i) {
            ...
            // Mixed hyperbolic-parabolic damping of psi.
